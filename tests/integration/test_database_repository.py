...
from chillax.adapters.database_repository import SqlAlchemyRepository
...
def test_repository_can_retrieve_tags(session_factory):
    ...
    tag_three = [tag for tag in tags if tag.tag_name == 'Mystery'][0]
    tag_four = [tag for tag in tags if tag.tag_name == 'Horror'][0]

    assert tag_one.number_of_tagged_articles == 303
    assert tag_two.number_of_tagged_articles == 120
    assert tag_three.number_of_tagged_articles == 106
    assert tag_four.number_of_tagged_articles == 119
