...
from chillax.domain.model import User, Movies, Comment, Tag, make_comment, make_tag_association
...
def insert_tags(empty_session):
    empty_session.execute(
        'INSERT INTO tags (name) VALUES ("Action"), ("Thriller")'
    )
...
def test_loading_of_article(empty_session):
    ...
    fetched_article = empty_session.query(Movies).one()
...
def test_loading_of_tagged_article(empty_session):
    ...
    article = empty_session.query(Movies).get(article_key)
...
def test_loading_of_commented_article(empty_session):
    ...
    rows = empty_session.query(Movies).all()
    article = rows[0]
