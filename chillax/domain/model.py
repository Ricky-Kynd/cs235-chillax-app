...
class Comment:
    def __init__(
            self, user: User, article: 'Movies', comment: str, timestamp: datetime
    ):
        self._user: User = user
        self._article: Movies = article
...
class Movies:
    ...
    def __repr__(self):
        return f'<Movies {self._date.isoformat()} {self._title}>'

    def __eq__(self, other):
        if not isinstance(other, Movies):
            return False
        return (
...
class Tag:
    def __init__(
        ...
        self._tagged_articles: List[Movies] = list()
    ...
    @property
    def tagged_articles(self) -> Iterable[Movies]:
    ...
    def is_applied_to(self, article: Movies) -> bool:
...
def make_comment(comment_text: str, user: User, article: Movies, timestamp: datetime = datetime.today()):
...
def make_tag_association(article: Movies, tag: Tag):
    if tag.is_applied_to(article):
        raise ModelException(f'Tag {tag.tag_name} already applied to Movies "{article.title}"')
