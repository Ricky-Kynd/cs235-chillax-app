...
from chillax.domain.model import make_comment, Movies, Comment, Tag


class NonExistentMoviesException(Exception):
    pass
...
def add_comment(article_id: int, comment_text: str, username: str, repo: AbstractRepository):
    ...
    if article is None:
        raise NonExistentMoviesException
...
def article_to_dict(article: Movies):
    article_dict = {
        'id': article.id,
        'date': article.date,
        'title': article.title,
...
def articles_to_dict(articles: Iterable[Movies]):
...
def dict_to_article(dict):
    article = Movies(dict.id, dict.date, dict.title, dict.first_para, dict.hyperlink)
